...
def generate_ledger(journal_df):
    """
    Generates ledgers from the journal entries.
    """
    if journal_df.empty:
        return pd.DataFrame()

    ledger_df = journal_df.copy()
    
    # Group by Account to create individual ledgers
    grouped_ledgers = {account: group for account, group in ledger_df.groupby('Account')}
...
def generate_trial_balance(ledger_df):
    ...
    # Get the last balance for each account
    account_balances = ledger_df.groupby('Account').last()['Balance'].reset_index()
    account_balances.rename(columns={'Balance': 'Final Balance'}, inplace=True)
...
st.markdown("""
This application allows you to upload and clean financial datasets. You can then use the cleaned data to automatically generate basic financial reports like a Journal, Ledgers, and a Trial Balance.
""")
st.markdown("---")
...
with ledger_col:
    if st.button("📒 Generate Ledgers"):
        with st.spinner("Generating ledgers..."):
            if 'journal_df' in st.session_state and not st.session_state.journal_df.empty:
                st.session_state.ledger_df = generate_ledger(st.session_state.journal_df)
                if not st.session_state.ledger_df.empty:
                    st.success("Ledgers generated successfully!")
                    st.dataframe(st.session_state.ledger_df)
                else:
    ...
    if 'ledger_df' in st.session_state and not st.session_state.ledger_df.empty:
        if st.button("💾 Store Ledgers"):
            with st.spinner("Storing ledgers to database..."):
                store_dataframe_to_db(st.session_state.ledger_df, "general_ledgers")
...
with tb_col:
    if st.button("⚖️ Generate Trial Balance"):
        with st.spinner("Generating trial balance..."):
            if 'ledger_df' in st.session_state and not st.session_state.ledger_df.empty:
                st.session_state.tb_df = generate_trial_balance(st.session_state.ledger_df)
